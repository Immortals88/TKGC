...
class TeLM(TKBCModel):
    ...
    def __init__(self, sizes: Tuple[int, int, int, int], rank: int,
                 no_time_emb=False, init_size: float = 1e-2, time_granularity: int = 1, pre_train: bool = True):
    ...
    def score(self, x):
        ...
        lhs = lhs[:, :self.rank], lhs[:, self.rank:self.rank * 2], lhs[:, self.rank * 2:self.rank * 3], lhs[:,
                                                                                                        self.rank * 3:]
        rel = rel[:, :self.rank], rel[:, self.rank:self.rank * 2], rel[:, self.rank * 2:self.rank * 3], rel[:,
                                                                                                        self.rank * 3:]
        rhs = rhs[:, :self.rank], rhs[:, self.rank:self.rank * 2], rhs[:, self.rank * 2:self.rank * 3], rhs[:,
                                                                                                        self.rank * 3:]
        time = time[:, :self.rank], time[:, self.rank:self.rank * 2], time[:, self.rank * 2:self.rank * 3], time[:,
                                                                                                            self.rank * 3:]
    ...
    def pretrain(self, x):
        ...
        return (
               ...
               ), self.embeddings[2].weight[:-1] if self.no_time_emb else torch.cat(
            (self.embeddings[2].weight[:, :self.rank], self.embeddings[2].weight[:,
                                                       self.rank * 3:]), dim=1)
    ...
    def forward(self, x):
        ...
        to_score = self.embeddings[0].weight
        to_score = to_score[:, :self.rank], to_score[:, self.rank:self.rank * 2], to_score[:,
                                                                                  self.rank * 2:self.rank * 3], to_score[
                                                                                                                :,
                                                                                                                self.rank * 3:]
