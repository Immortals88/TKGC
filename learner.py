...
parser.add_argument('--model', default='TeLM', type=str,
    help="Model Name")
...
parser.add_argument('--time_reg', default=0., type=float,
    help="Timestamp regularizer strength")
parser.add_argument('--no_time_emb', default=False, action="store_true",
    help="Use a specific embedding for non temporal relations")
parser.add_argument('--time_granularity', default=1, type=int,
...
def learn(model=args.model,
    ...
    PATH=os.path.join(root,'rank{:.0f}/lr{:.4f}/batch{:.0f}/time_granularity{:02d}/emb_reg{:.5f}/time_reg{:.5f}/'.format(
        rank, learning_rate, batch_size, time_granularity, emb_reg, time_reg))
    
    dataset = TemporalDataset(dataset)
    
    sizes = dataset.get_shape()
    model = {'TeLM': TeLM(sizes, rank, no_time_emb=args.no_time_emb, time_granularity=time_granularity)}[model]
    model = model.cuda()
    ...
    embedding = model.embeddings[0].weight.data.cpu()
    links = load_alignment_pair('fuck/ref_pairs')
    links = torch.tensor(links)
    left = torch.tensor(links[:, 0])
    right = torch.tensor(links[:, 1])
    l_emb = embedding[left]
    r_emb = embedding[right]
    # get_hits_slow(l_emb, r_emb, [(i, i) for i in range(len(links))])
    linear_reg = linear_model.LinearRegression()
    linear_reg.fit(l_emb, r_emb)
    joblib.dump(linear_reg, 'linear.pkl')
    linear_reg = joblib.load('linear.pkl')
    y_predict = linear_reg.predict(l_emb)

    get_hits_slow(l_emb.numpy(), y_predict, [(i, i) for i in range(len(links))])
